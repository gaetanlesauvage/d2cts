...
public class NewMissionOrderDialog implements ActionListener {
	private JDialog jd;

	public static int getChoice(ContainerLocation currentLocation,
			String currentExceptionMessage) {
		NewMissionOrderDialog nmod = new NewMissionOrderDialog(currentLocation,
				currentExceptionMessage);
		nmod.waitTillOkIsPress();

		if (nmod.jrbAbort.isSelected())
	...
	private JRadioButton jrbAbort, jrbGoToOrigin, jrbGoToDest;

	private boolean isReady = false;

	public NewMissionOrderDialog(final ContainerLocation currentLocation,
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				c.ipadx = 20;
				c.gridwidth = GridBagConstraints.REMAINDER;
				jrbAbort = new JRadioButton("Abort mission");
				jrbAbort.addActionListener(aListener);
				contentPane.add(jrbAbort, c);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jrbAbort || e.getSource() == jrbGoToDest
