...
public class YDistanceHeuristic implements AStarHeuristic {
	public static final String NAME = "YDistanceHeuristic";
	private static final double GAMA = 0.5;
	private SpeedCharacteristics speed;

	public YDistanceHeuristic(SpeedCharacteristics speed) {
		this.speed = speed;
	}
	...
	public double heuristic(RoadPoint from, RoadPoint to) {
		return Math.sqrt(GAMA
				* Math.pow(from.getLocation().y - to.getLocation().y, 2)
				+ (1.0 - GAMA)
				* Math.pow(from.getLocation().x - to.getLocation().x, 2));
		// Location.getLength(from.getLocation(), to.getLocation());
	}
